...
版心縮小，使得邊注區產生更大的空間。
\par 111 行和112 行 將頁面還原囘標準紙。
\par 113 行定義 baseline 為 30 pt。
\par 114 行，定義的系統文字縮放比例為 0.924690
...
\else % 無糸欄、無邉框
	\node [right,]  at%
		(1.2,1.7) {\hbox{\yoko\mgfamily\small\hbox{\kanjiskip=1pt(*版權所有*)}%
		\qquad\hbox{\kanjiskip=1pt(*翻印必究*)}}};
	\@ppageodd
\fi
\end{tikzpicture}%
...
\else % 無糸欄、無邉框
	\node [left,]  at%
		(17,1.7) {\hbox{\yoko\mgfamily\small\hbox{\kanjiskip=1pt(*版權所有*)}%
		\qquad\hbox{\kanjiskip=1pt(*翻印必究*)}}};
	\@ppageven
\fi
	\end{tikzpicture}%
...
\begin{lstlisting}[firstnumber=1181]
...
\begin{lstlisting}[firstnumber=1194]
...
\begin{lstlisting}[firstnumber=1211]
...
\begin{lstlisting}[firstnumber=1255]
...
\begin{lstlisting}[firstnumber=1296]
\newcommand*{\l@part}[2]{%
...
\begin{lstlisting}[firstnumber=1353]
%% 圖片目錄
...
\begin{lstlisting}[firstnumber=1391]
...
\begin{lstlisting}[firstnumber=1458]
\newdimen\endnotesep
